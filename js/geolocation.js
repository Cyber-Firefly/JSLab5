...
function success(position) {
  ...
  let url = `https://www.openstreetmap.org/#map=${position.coords.latitude}/${position.coords.longitude}`;
  mapLink.setAttribute("href", url);
  mapLink.setAttribute("target", "_blank");
  mapLink.textContent = "Open on Maps!";
  statusMsg.textContent = "You are found!";
}

// STEP 4a: Construct the error() function
function error() {
  // STEP 4b: Output a suitable error message
  statusMsg.textContent = "Sorry, location is not found.";
}
